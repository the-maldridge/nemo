...
var EventWhere = struct {
	...
	ID:          whereHelpernull_Int64{field: "\"events\".\"id\""},
	Title:       whereHelperstring{field: "\"events\".\"title\""},
	Description: whereHelperstring{field: "\"events\".\"description\""},
	Archived:    whereHelperbool{field: "\"events\".\"archived\""},
	StreamID:    whereHelperint64{field: "\"events\".\"stream_id\""},
}
...
type eventR struct {
	Stream *Stream `boil:"Stream" json:"Stream" toml:"Stream" yaml:"Stream"`
}
...
var (
	eventAllColumns            = []string{"id", "title", "description", "archived", "stream_id"}
	eventColumnsWithoutDefault = []string{}
	eventColumnsWithDefault    = []string{"id", "title", "description", "archived", "stream_id"}
...
// Stream pointed to by the foreign key.
func (o *Event) Stream(mods ...qm.QueryMod) streamQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"id\" = ?", o.StreamID),
	}

	queryMods = append(queryMods, mods...)

	query := Streams(queryMods...)
	queries.SetFrom(query.Query, "\"streams\"")

	return query
}

// LoadStream allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (eventL) LoadStream(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEvent interface{}, mods queries.Applicator) error {
	var slice []*Event
	var object *Event
	...
	if singular {
		...
		if !queries.IsNil(object.StreamID) {
			args = append(args, object.StreamID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.StreamID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.StreamID) {
				args = append(args, obj.StreamID)
			}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for streams")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.StreamID, foreign.ID) {
				local.R.Stream = foreign
				if foreign.R == nil {
					foreign.R = &streamR{}
				}
...
// SetStream of the event to the related item.
// Sets o.R.Stream to related.
// Adds o to related.R.Events.
func (o *Event) SetStream(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Stream) error {
	var err error
	if insert {
	...
	queries.Assign(&o.StreamID, related.ID)
	if o.R == nil {
		o.R = &eventR{
			Stream: related,
		}
...
func FindEvent(ctx context.Context, exec boil.ContextExecutor, iD null.Int64, selectCols ...string) (*Event, error) {
	eventObj := &Event{}
...
func EventExists(ctx context.Context, exec boil.ContextExecutor, iD null.Int64) (bool, error) {
	var exists bool
