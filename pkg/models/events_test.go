...
func testEventToOneStreamUsingStream(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, streamDBTypes, true, streamColumnsWithDefault...); err != nil {
	...
	queries.Assign(&local.StreamID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Stream().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadStream(ctx, tx, false, (*[]*Event)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Stream == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Stream = nil
	if err = local.L.LoadStream(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Stream == nil {
		t.Error("struct should have been eager loaded")
...
func testEventToOneSetOpStreamUsingStream(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, streamDBTypes, false, strmangle.SetComplement(streamPrimaryKeyColumns, streamColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, streamDBTypes, false, strmangle.SetComplement(streamPrimaryKeyColumns, streamColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Stream{&b, &c} {
		err = a.SetStream(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.StreamID, x.ID) {
			t.Error("foreign key was wrong value", a.StreamID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.StreamID))
		reflect.Indirect(reflect.ValueOf(&a.StreamID)).Set(zero)
		...
		if !queries.Equal(a.StreamID, x.ID) {
			t.Error("foreign key was wrong value", a.StreamID, x.ID)
		}
	}
}
...
var (
	eventDBTypes = map[string]string{`ID`: `INTEGER`, `Title`: `VARCHAR (512)`, `Description`: `VARCHAR (4096)`, `Archived`: `BOOLEAN`, `StreamID`: `INTEGER`}
	_            = bytes.MinRead
)
